...
QsciCommandSet::QsciCommandSet(QsciScintilla *qs) : qsci(qs) {
  ...
  static struct sci_cmd cmd_table[] = {
      ...
      {QsciCommand::LineDownExtend, Qt::Key_Down | qHash(Qt::SHIFT),
#if defined(USING_OSX_KEYS)
       Qt::Key_N | Qt::META | qHash(Qt::SHIFT),
#else
       0,
#endif
       QT_TRANSLATE_NOOP("QsciCommand", "Extend selection down one line")},
      {QsciCommand::LineDownRectExtend,
       Qt::Key_Down | qHash(Qt::ALT) | qHash(Qt::SHIFT),
#if defined(USING_OSX_KEYS)
       Qt::Key_N | Qt::META | qHash(Qt::ALT) | qHash(Qt::SHIFT),
#else
       0,
#endif
       QT_TRANSLATE_NOOP("QsciCommand",
                         "Extend rectangular selection down one line")},
      {QsciCommand::LineScrollDown, Qt::Key_Down | qHash(Qt::CTRL), 0,
      ...
      {QsciCommand::LineUpExtend, Qt::Key_Up | qHash(Qt::SHIFT),
#if defined(USING_OSX_KEYS)
       Qt::Key_P | Qt::META | qHash(Qt::SHIFT),
#else
       0,
#endif
       QT_TRANSLATE_NOOP("QsciCommand", "Extend selection up one line")},
      {QsciCommand::LineUpRectExtend,
       Qt::Key_Up | qHash(Qt::ALT) | qHash(Qt::SHIFT),
#if defined(USING_OSX_KEYS)
       Qt::Key_P | Qt::META | qHash(Qt::ALT) | qHash(Qt::SHIFT),
#else
       0,
#endif
       QT_TRANSLATE_NOOP("QsciCommand",
                         "Extend rectangular selection up one line")},
      {QsciCommand::LineScrollUp, Qt::Key_Up | qHash(Qt::CTRL), 0,
      ...
      {QsciCommand::CharLeftExtend, Qt::Key_Left | qHash(Qt::SHIFT),
#if defined(USING_OSX_KEYS)
       Qt::Key_B | Qt::META | qHash(Qt::SHIFT),
#else
       0,
#endif
       QT_TRANSLATE_NOOP("QsciCommand", "Extend selection left one character")},
      {QsciCommand::CharLeftRectExtend,
       Qt::Key_Left | qHash(Qt::ALT) | qHash(Qt::SHIFT),
#if defined(USING_OSX_KEYS)
       Qt::Key_B | Qt::META | qHash(Qt::ALT) | qHash(Qt::SHIFT),
#else
       0,
#endif
       QT_TRANSLATE_NOOP("QsciCommand",
      ...
      {QsciCommand::CharRightExtend, Qt::Key_Right | qHash(Qt::SHIFT),
#if defined(USING_OSX_KEYS)
       Qt::Key_F | Qt::META | qHash(Qt::SHIFT),
#else
       0,
#endif
       QT_TRANSLATE_NOOP("QsciCommand",
      ...
      {QsciCommand::CharRightRectExtend,
       Qt::Key_Right | qHash(Qt::ALT) | qHash(Qt::SHIFT),
#if defined(USING_OSX_KEYS)
       Qt::Key_F | Qt::META | qHash(Qt::ALT) | qHash(Qt::SHIFT),
#else
       0,
#endif
       QT_TRANSLATE_NOOP("QsciCommand",
...
#if defined(USING_OSX_KEYS)
       Qt::Key_Left | qHash(Qt::ALT),
#else
       Qt::Key_Left | qHash(Qt::CTRL),
#endif
      ...
      {QsciCommand::WordLeftExtend,
       ...
       Qt::Key_Left | qHash(Qt::ALT) | qHash(Qt::SHIFT),
#else
       Qt::Key_Left | qHash(Qt::CTRL) | qHash(Qt::SHIFT),
#endif
      ...
      {QsciCommand::WordPartLeft, Qt::Key_Slash | qHash(Qt::CTRL), 0,
       QT_TRANSLATE_NOOP("QsciCommand", "Move left one word part")},
      {QsciCommand::WordPartLeftExtend,
       Qt::Key_Slash | qHash(Qt::CTRL) | qHash(Qt::SHIFT), 0,
       QT_TRANSLATE_NOOP("QsciCommand", "Extend selection left one word part")},
      {QsciCommand::WordPartRight, Qt::Key_Backslash | qHash(Qt::CTRL), 0,
       QT_TRANSLATE_NOOP("QsciCommand", "Move right one word part")},
      {QsciCommand::WordPartRightExtend,
       Qt::Key_Backslash | qHash(Qt::CTRL) | qHash(Qt::SHIFT), 0,
       QT_TRANSLATE_NOOP("QsciCommand",
...
#if defined(USING_OSX_KEYS)
       Qt::Key_A | Qt::META | qHash(Qt::SHIFT),
#else
       0,
#endif
       0,
...
#if defined(USING_OSX_KEYS)
       Qt::Key_A | Qt::META | qHash(Qt::ALT) | qHash(Qt::SHIFT),
#else
       0,
#endif
       0,
      ...
      {QsciCommand::VCHomeRectExtend,
       ...
       0,
#else
       Qt::Key_Home | qHash(Qt::ALT) | qHash(Qt::SHIFT),
#endif
       0,
       QT_TRANSLATE_NOOP("QsciCommand",
                         "Extend rectangular selection to first visible "
                         "character in document line")},
...
#if defined(USING_OSX_KEYS)
       Qt::Key_E | Qt::META | qHash(Qt::SHIFT),
#else
       Qt::Key_End | qHash(Qt::SHIFT),
#endif
       0,
       QT_TRANSLATE_NOOP("QsciCommand",
...
#if defined(USING_OSX_KEYS)
       Qt::Key_E | Qt::META | qHash(Qt::ALT) | qHash(Qt::SHIFT),
#else
       Qt::Key_End | qHash(Qt::ALT) | qHash(Qt::SHIFT),
#endif
       0,
       QT_TRANSLATE_NOOP(
           "QsciCommand",
...
#if defined(USING_OSX_KEYS)
       Qt::Key_Up | qHash(Qt::CTRL),
#else
       Qt::Key_Home | qHash(Qt::CTRL),
#endif
       0, QT_TRANSLATE_NOOP("QsciCommand", "Move to start of document")},
      {QsciCommand::DocumentStartExtend,
#if defined(USING_OSX_KEYS)
       Qt::Key_Up | qHash(Qt::CTRL) | qHash(Qt::SHIFT),
#else
       Qt::Key_Home | qHash(Qt::CTRL) | qHash(Qt::SHIFT),
#endif
       0,
       QT_TRANSLATE_NOOP("QsciCommand",
...
#if defined(USING_OSX_KEYS)
       Qt::Key_Down | qHash(Qt::CTRL),
#else
       Qt::Key_End | qHash(Qt::CTRL),
#endif
       0, QT_TRANSLATE_NOOP("QsciCommand", "Move to end of document")},
      {QsciCommand::DocumentEndExtend,
#if defined(USING_OSX_KEYS)
       Qt::Key_Down | qHash(Qt::CTRL) | qHash(Qt::SHIFT),
#else
       Qt::Key_End | qHash(Qt::CTRL) | qHash(Qt::SHIFT),
#endif
       0,
       QT_TRANSLATE_NOOP("QsciCommand", "Extend selection to end of document")},
      {QsciCommand::PageUp, Qt::Key_PageUp, 0,
       QT_TRANSLATE_NOOP("QsciCommand", "Move up one page")},
      {QsciCommand::PageUpExtend, Qt::Key_PageUp | qHash(Qt::SHIFT), 0,
       QT_TRANSLATE_NOOP("QsciCommand", "Extend selection up one page")},
      {QsciCommand::PageUpRectExtend,
       Qt::Key_PageUp | qHash(Qt::ALT) | qHash(Qt::SHIFT), 0,
       QT_TRANSLATE_NOOP("QsciCommand",
      ...
      {QsciCommand::PageDownExtend, Qt::Key_PageDown | qHash(Qt::SHIFT),
#if defined(USING_OSX_KEYS)
       Qt::Key_V | Qt::META | qHash(Qt::SHIFT),
#else
       0,
#endif
       QT_TRANSLATE_NOOP("QsciCommand", "Extend selection down one page")},
      {QsciCommand::PageDownRectExtend,
       Qt::Key_PageDown | qHash(Qt::ALT) | qHash(Qt::SHIFT),
#if defined(USING_OSX_KEYS)
       Qt::Key_V | Qt::META | qHash(Qt::ALT) | qHash(Qt::SHIFT),
#else
       0,
#endif
       QT_TRANSLATE_NOOP("QsciCommand",
                         "Extend rectangular selection down one page")},
      {QsciCommand::StutteredPageUp, 0, 0,
       QT_TRANSLATE_NOOP("QsciCommand", "Stuttered move up one page")},
      {QsciCommand::StutteredPageUpExtend, 0, 0,
       QT_TRANSLATE_NOOP("QsciCommand",
                         "Stuttered extend selection up one page")},
      {QsciCommand::StutteredPageDown, 0, 0,
       QT_TRANSLATE_NOOP("QsciCommand", "Stuttered move down one page")},
      {QsciCommand::StutteredPageDownExtend, 0, 0,
       QT_TRANSLATE_NOOP("QsciCommand",
      ...
      {QsciCommand::DeleteBack, Qt::Key_Backspace,
       ...
       Qt::Key_H | Qt::META,
#else
       Qt::Key_Backspace | qHash(Qt::SHIFT),
#endif
      ...
      {QsciCommand::DeleteWordLeft, Qt::Key_Backspace | qHash(Qt::CTRL), 0,
       QT_TRANSLATE_NOOP("QsciCommand", "Delete word to left")},
      {QsciCommand::DeleteWordRight, Qt::Key_Delete | qHash(Qt::CTRL), 0,
       QT_TRANSLATE_NOOP("QsciCommand", "Delete word to right")},
      {QsciCommand::DeleteWordRightEnd,
#if defined(USING_OSX_KEYS)
       Qt::Key_Delete | qHash(Qt::ALT),
#else
       0,
#endif
       0, QT_TRANSLATE_NOOP("QsciCommand", "Delete right to end of next word")},
      {QsciCommand::DeleteLineLeft,
       Qt::Key_Backspace | qHash(Qt::CTRL) | qHash(Qt::SHIFT), 0,
       QT_TRANSLATE_NOOP("QsciCommand", "Delete line to left")},
      {QsciCommand::DeleteLineRight,
#if defined(USING_OSX_KEYS)
       Qt::Key_K | Qt::META,
#else
       Qt::Key_Delete | qHash(Qt::CTRL) | qHash(Qt::SHIFT),
#endif
       0, QT_TRANSLATE_NOOP("QsciCommand", "Delete line to right")},
      {QsciCommand::LineDelete, Qt::Key_L | qHash(Qt::CTRL) | qHash(Qt::SHIFT),
       0, QT_TRANSLATE_NOOP("QsciCommand", "Delete current line")},
      {QsciCommand::LineCut, Qt::Key_L | qHash(Qt::CTRL), 0,
       QT_TRANSLATE_NOOP("QsciCommand", "Cut current line")},
      {QsciCommand::LineCopy, Qt::Key_T | qHash(Qt::CTRL) | qHash(Qt::SHIFT), 0,
       QT_TRANSLATE_NOOP("QsciCommand", "Copy current line")},
      {QsciCommand::LineTranspose, Qt::Key_T | qHash(Qt::CTRL), 0,
       QT_TRANSLATE_NOOP("QsciCommand",
                         "Transpose current and previous lines")},
      {QsciCommand::LineDuplicate, 0, 0,
       QT_TRANSLATE_NOOP("QsciCommand", "Duplicate the current line")},
      {QsciCommand::SelectAll, Qt::Key_A | qHash(Qt::CTRL), 0,
       QT_TRANSLATE_NOOP("QsciCommand", "Select all")},
      {QsciCommand::MoveSelectedLinesUp, 0, 0,
       QT_TRANSLATE_NOOP("QsciCommand", "Move selected lines up one line")},
      {QsciCommand::MoveSelectedLinesDown, 0, 0,
       QT_TRANSLATE_NOOP("QsciCommand", "Move selected lines down one line")},
      {QsciCommand::SelectionDuplicate, Qt::Key_D | qHash(Qt::CTRL), 0,
       QT_TRANSLATE_NOOP("QsciCommand", "Duplicate selection")},
      {QsciCommand::SelectionLowerCase, Qt::Key_U | qHash(Qt::CTRL), 0,
       QT_TRANSLATE_NOOP("QsciCommand", "Convert selection to lower case")},
      {QsciCommand::SelectionUpperCase,
       Qt::Key_U | qHash(Qt::CTRL) | qHash(Qt::SHIFT), 0,
       QT_TRANSLATE_NOOP("QsciCommand", "Convert selection to upper case")},
      {QsciCommand::SelectionCut, Qt::Key_X | qHash(Qt::CTRL),
       Qt::Key_Delete | qHash(Qt::SHIFT),
       QT_TRANSLATE_NOOP("QsciCommand", "Cut selection")},
      {QsciCommand::SelectionCopy, Qt::Key_C | qHash(Qt::CTRL),
       Qt::Key_Insert | qHash(Qt::CTRL),
       QT_TRANSLATE_NOOP("QsciCommand", "Copy selection")},
      {QsciCommand::Paste, Qt::Key_V | qHash(Qt::CTRL),
       Qt::Key_Insert | qHash(Qt::SHIFT),
      ...
      {QsciCommand::Newline, Qt::Key_Return, Qt::Key_Return | qHash(Qt::SHIFT),
      ...
      {QsciCommand::Backtab, Qt::Key_Tab | qHash(Qt::SHIFT), 0,
       QT_TRANSLATE_NOOP("QsciCommand", "De-indent one level")},
      {QsciCommand::Cancel, Qt::Key_Escape, 0,
       QT_TRANSLATE_NOOP("QsciCommand", "Cancel")},
      {QsciCommand::Undo, Qt::Key_Z | qHash(Qt::CTRL),
       Qt::Key_Backspace | qHash(Qt::ALT),
       QT_TRANSLATE_NOOP("QsciCommand", "Undo last command")},
      {QsciCommand::Redo,
#if defined(USING_OSX_KEYS)
       Qt::Key_Z | qHash(Qt::CTRL) | qHash(Qt::SHIFT),
#else
       Qt::Key_Y | qHash(Qt::CTRL),
#endif
       0, QT_TRANSLATE_NOOP("QsciCommand", "Redo last command")},
      {QsciCommand::ZoomIn, Qt::Key_Plus | qHash(Qt::CTRL), 0,
       QT_TRANSLATE_NOOP("QsciCommand", "Zoom in")},
      {QsciCommand::ZoomOut, Qt::Key_Minus | qHash(Qt::CTRL), 0,
  ...
  for (int i = 0; i < sizeof(cmd_table) / sizeof(cmd_table[0]); ++i)
    cmds.append(new QsciCommand(qsci, cmd_table[i].cmd, cmd_table[i].key,
                                cmd_table[i].altkey, cmd_table[i].desc));
}

// The dtor.
QsciCommandSet::~QsciCommandSet() {
  for (int i = 0; i < cmds.count(); ++i) delete cmds.at(i);
}
...
bool QsciCommandSet::readSettings(QSettings &qs, const char *prefix) {
  ...
  for (int i = 0; i < cmds.count(); ++i) {
    ...
    QString skey = QString("%1/keymap/c%2/")
                       .arg(prefix)
                       .arg(static_cast<int>(cmd->command()));
...
void QsciCommandSet::clearKeys() {
  for (int i = 0; i < cmds.count(); ++i) cmds.at(i)->setKey(0);
}

// Clear the alternate key bindings.
void QsciCommandSet::clearAlternateKeys() {
  for (int i = 0; i < cmds.count(); ++i) cmds.at(i)->setAlternateKey(0);
}
...
QsciCommand *QsciCommandSet::boundTo(int key) const {
  for (int i = 0; i < cmds.count(); ++i) {
    ...
    if (cmd->key() == key || cmd->alternateKey() == key) return cmd;
  }
