...
YDAddTempVarDialog::YDAddTempVarDialog(QWidget *parent)
    : YDDialog{parent},
      ...
      m_varDlg{nullptr},
      m_isUpdate{false} {
  setWindowTitle(YDAddTempVarDialog::tr("添加临时变量"));
...
void YDAddTempVarDialog::setTextList(const QStringList &infos) {
  setWindowTitle(YDAddTempVarDialog::tr("修改临时变量"));
...
void YDAddTempVarDialog::setUpdateParam(const QModelIndex &groupIndex,
...
void YDAddTempVarDialog::slotOkBtnClicked() {
  ...
  if (m_isUpdate) {
    ...
    if (row < vars.size()) {
      ...
      switch (result) {
        case -1:
          QMessageBox::warning(nullptr, YDAddTempVarDialog::tr("提示"),
                               YDAddTempVarDialog::tr("修改临时变量名称失败!"));
          return;
        case 0:
          break;
        case 1:
          QMessageBox::warning(nullptr, YDAddTempVarDialog::tr("提示"),
                               YDAddTempVarDialog::tr("临时变量名称有重复!"));
          return;
        default:
          return;
      }
